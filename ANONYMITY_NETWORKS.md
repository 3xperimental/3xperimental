# Anonymity Networks with 3xperimental

Currently only Tor and I2P have been integrated into 3xperimental. The usage of
...
is made or until `3xperimentald` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `3xperimental-wallet-cli` and
`3xperimental-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--proxy` which tells `3xperimentald` the ip/port of a socks proxy provided by a
separate process. On most systems the configuration will look like:

> `--proxy tor,127.0.0.1:9050,10`
> `--proxy i2p,127.0.0.1:9000`

which tells `3xperimentald` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `3xperimentald` the inbound address, network
type, and max connections:

> `--anonymous-inbound rveahdfho7wo4b2m.onion:29996,127.0.0.1:29996,25`
> `--anonymous-inbound cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000,127.0.0.1:30000`

which tells `3xperimentald` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:29996" and forwarded to `3xperimentald`
...
forwarded to `3xperimentald` localhost port 30000.
...
`3xperimentald` RPC port - which is independent from the configuration for incoming
...
> HiddenServiceDir /var/lib/tor/data/3xperimental
...
This will store key information in `/var/lib/tor/data/3xperimental` and will forward
"Tor port" 29996 to port 29996 of ip 127.0.0.1. The file
`/usr/lib/tor/data/3xperimental/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the 3xperimental-Network should have
similar timestamps.
...
An ISP can passively monitor `3xperimentald` connections from a node and observe when
...
only using I2P for broadcasting 3xperimental transactions, the total aggregate of
I2P data would also leak information.

#### Mitigation

There is no current mitigation for the user right now. This attack is fairly
sophisticated, and likely requires support from the internet host of an 3xperimental
user.
...
### Intermittent 3xperimental Syncing

If a user only runs `3xperimentald` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `3xperimentald` as often as possible to conceal when transactions are being sent.
